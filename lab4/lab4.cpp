...
// ================================================================
//  Лапласиан — конечно-разностные ядра в строгом математическом смысле
//  Δf = d²f/dx² + d²f/dy²  (сумма вторых производных по осям)
//
//  Ядро крестообразное: ненулевые значения только в центральной строке
//  и центральном столбце. Для каждого размера используется стандартный
//  1D стенсиль вычисления d²f/dx²:
//    size=3 (3-точечный, 2-й порядок):   [1, -2, 1]
//    size=5 (5-точечный, 4-й порядок):   [-1, 16, -30, 16, -1] / 12
//    size=7 (7-точечный, 6-й порядок):   [2, -27, 270, -490, 270, -27, 2] / 180
//  Одномерные стенсили складываются по x и y → 2D Лапласиан.
//  Результат смещается на +128 для визуализации (серый = нет отклика).
// ================================================================
    ...
    size_t imgW, size_t imgH, int size)
{
    ...
    // 1D конечно-разностный стенсиль для d²/dx² (и d²/dy²)
    std::vector<double> s(size, 0.0);
    if (size == 3) {
        // 3-точечный, 2-й порядок точности
        s = { 1.0, -2.0, 1.0 };
    } else if (size == 5) {
        // 5-точечный, 4-й порядок точности: [-1, 16, -30, 16, -1] / 12
        s = { -1.0/12, 16.0/12, -30.0/12, 16.0/12, -1.0/12 };
    } else if (size == 7) {
        // 7-точечный, 6-й порядок точности: [2,-27,270,-490,270,-27,2] / 180
        s = { 2.0/180, -27.0/180, 270.0/180, -490.0/180, 270.0/180, -27.0/180, 2.0/180 };
    } else {
        // Для произвольного размера используем 3-точечный стенсиль в центре
        s[half - 1] = 1.0;
        s[half]     = -2.0;
        s[half + 1] = 1.0;
    }

    // Строим 2D Лапласиан: d²f/dx² (вдоль центральной строки)
    //                     + d²f/dy² (вдоль центрального столбца)
    for (int k = 0; k < size; ++k) {
        kernel[half * size + k] += s[k];  // d²/dx²: центральная строка
        kernel[k * size + half] += s[k];  // d²/dy²: центральный столбец
    }
    // Пиксель в центре входит в обе суммы → это правильно:
    // kernel[center] = s[half] + s[half] = -4 (для 3×3), что совпадает
    // с классическим дискретным Лапласианом [0,1,0;1,-4,1;0,1,0]

    // Применяем крестообразное ядро, смещаем результат на +128
    for (int y = 0; y < (int)imgH; ++y) {
        for (int x = 0; x < (int)imgW; ++x) {
            double sum = 0.0;
